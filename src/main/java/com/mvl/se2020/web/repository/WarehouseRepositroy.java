...
public interface WarehouseRepositroy extends JpaRepository<Warehouse, Long> {
	...
	@Query(value = "Select * from warehouse where location=:location and warehouse_name like %:name% and status='ENABLE'", nativeQuery = true)
	List<Warehouse> getByNameAndLocation(String location, String name);
	...
	@Query(value = "Select * from warehouse where warehouse_id=?1 and status='ENABLE'", nativeQuery = true)
	Warehouse getWareById(Long wareId);
